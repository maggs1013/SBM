...
def expected_calibration_error(y_true, proba, n_bins=15):
    y = np.array(y_true); P = np.array(proba)
    bins = np.linspace(0,1,n_bins+1); eces=[]
    for k in range(3):
        pk = P[:,k]; yk = (y==k).astype(float)
        for i in range(n_bins):
            m = (pk>=bins[i]) & (pk<bins[i+1])
            if m.sum()<30: continue
            eces.append(abs(yk[m].mean() - pk[m].mean()) * m.mean())
...
def main():
    ...
    k = int(cfg.get('skellam',{}).get('goal_diff_range',10))
    ...
    # Skellam using xg
    df['pH_raw'], df['pD_raw'], df['pA_raw'] = zip(*df.apply(lambda r: skellam_probs(r.get('xg_home', np.nan),
                                                                                      r.get('xg_away', np.nan), k), axis=1))
    # If xG missing, use fair probs as raw
    m = df[['pH_raw','pD_raw','pA_raw']].isna().any(axis=1)
    df.loc[m, ['pH_raw','pD_raw','pA_raw']] = df.loc[m, ['fair_home','fair_draw','fair_away']].values

    # Labels: A=0, D=1, H=2
    y = np.where(df['goals_home']>df['goals_away'], 2, np.where(df['goals_home']<df['goals_away'], 0, 1))

    # Calibrate per league (skip small)
    out_frames=[]
    for lg, g in df.groupby('league'):
        if len(g) < int(cfg.get('calibration',{}).get('min_samples',500)):
    ...
    # ROI vs fair odds (neutral benchmark)
    for col, new in [('fair_home','oddsH_fair'), ('fair_draw','oddsD_fair'), ('fair_away','oddsA_fair')]:
        df[new] = 1.0 / df[col].clip(1e-9, 1.0)

    def kelly(p, odds, frac):
        b = odds - 1.0
        edge = p*b - (1-p)
        if edge <= 0: return 0.0, edge
        k = edge / b
        return frac*k, edge

    stakes=[]; edges=[]
    for ph,pd_,pa,oh,od,oa in zip(df['pH_cal'], df['pD_cal'], df['pA_cal'], df['oddsH_fair'], df['oddsD_fair'], df['oddsA_fair']):
        sh,eh = kelly(ph,oh,frac); sd,ed = kelly(pd_,od,frac); sa,ea = kelly(pa,oa,frac)
        stakes.append((sh,sd,sa)); edges.append((eh,ed,ea))
    ...
    pd.DataFrame([{'rows': int(len(df)), 'ece_multiclass': float(ece_global), 'roi_fractional_kelly_vs_fair': roi}]).to_csv(reports/"BACKTEST_SUMMARY.csv", index=False)
